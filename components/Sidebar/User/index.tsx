...
const User = ({ isCollapsed }: Props) => {
    return (
        <div
            className={`py-4 border-t border-gray-100 max-md:py-0 max-md:px-5 ${isCollapsed ? "px-0" : "px-3"
                }`}
        >
            <Link
                className={`group flex items-center h-12 bg-gray-0 rounded-lg max-2xl:bg-gray-25 max-md:bg-transparent ${isCollapsed ? "justify-center" : "px-3"
                    }`}
                href="/auth/sign-in"
            >
